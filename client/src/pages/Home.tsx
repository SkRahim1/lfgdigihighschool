...
const Home = () => {
  ...
  return (
    <div className="overflow-hidden bg-slate-50 text-slate-800">
      {/* Hero Section */}
      <section className="relative bg-gradient-to-br from-slate-900 via-slate-800 to-indigo-950 py-20 lg:py-32 text-white border-b border-slate-800">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 flex flex-col lg:flex-row items-center gap-16">
          <div className="flex-1 text-center lg:text-left">
            <h1 className="text-4xl sm:text-5xl lg:text-6xl font-extrabold leading-tight mb-6 tracking-tight">
              Welcome to <span className="bg-gradient-to-r from-amber-400 to-yellow-300 bg-clip-text text-transparent">Lfg Digi High School</span>
            </h1>
            <p className="text-base sm:text-lg lg:text-xl text-slate-300 mb-8 max-w-2xl mx-auto lg:mx-0">
            ...
            <div className="flex flex-col sm:flex-row justify-center lg:justify-start gap-4">
              <Link 
                ...
                className="bg-school-secondary hover:bg-amber-500 text-slate-950 px-8 py-4 rounded-full font-extrabold text-lg hover:scale-[1.03] active:scale-95 transition-all flex items-center justify-center gap-2 shadow-lg shadow-school-secondary/15"
              >
                Inquire About Admission <ArrowRight size={20} />
              </Link>
              <Link 
                to="/about" 
                className="bg-white/5 border border-white/10 px-8 py-4 rounded-full font-bold text-lg hover:bg-white/10 active:scale-95 transition-all flex items-center justify-center"
              >
                Learn More
              </Link>
            </div>
      ...
      {/* Notices Ticker */}
      <div className="bg-slate-900 border-b border-slate-800 py-3.5 text-white overflow-hidden whitespace-nowrap">
        <div className="max-w-7xl mx-auto px-4 flex items-center gap-6">
          <span className="font-extrabold text-sm uppercase tracking-wider text-school-secondary flex items-center gap-2 bg-amber-400/10 px-3 py-1 rounded-lg shrink-0">
            <Bell size={16} /> Latest Notices:
          </span>
          <div className="flex-1 overflow-hidden relative w-full">
            <div className="flex animate-marquee items-center gap-8 text-slate-300 text-sm font-semibold">
              {notices.length > 0 ? notices.map(n => (
                <span key={n.id} className="hover:text-white transition-colors">{n.title} - {new Date(n.date).toLocaleDateString()} |</span>
              )) : (
      ...
      <section className="py-16 bg-slate-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="bg-gradient-to-r from-indigo-900 to-slate-900 rounded-3xl p-8 lg:p-16 flex flex-col lg:flex-row items-center justify-between gap-8 text-white shadow-xl relative overflow-hidden border border-white/5">
            <div className="absolute inset-0 bg-[radial-gradient(circle_at_70%_120%,rgba(168,85,247,0.15),transparent)] pointer-events-none" />
            <div className="relative z-10 text-center lg:text-left">
              <h3 className="text-3xl font-extrabold mb-4">Ready to join our community?</h3>
              <p className="text-lg text-slate-300">Admissions for the current session are closing soon. Don't miss out!</p>
            </div>
            <Link 
              to="/contact" 
              className="bg-white text-slate-900 px-8 py-4 rounded-full font-extrabold text-lg hover:bg-slate-100 hover:scale-[1.03] active:scale-95 transition-all shadow-lg shrink-0 relative z-10"
            >
              Contact Us Today
            </Link>
          </div>
...
const FeatureCard = ({ icon, iconBg, title, description }: FeatureCardProps) => (
  <div className="bg-white p-8 rounded-3xl shadow-sm hover:shadow-xl transition-all duration-300 border border-slate-100 hover:border-slate-200 group hover:-translate-y-1">
